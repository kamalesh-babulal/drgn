...
static int unpack_member(struct drgn_compound_type_builder *builder,
			 PyObject *cached_members_obj)
{
	TypeMember *item = (TypeMember *)PyTuple_GET_ITEM(cached_members_obj,
							  builder->members.size);
	...
	unsigned long long bit_offset =
		PyLong_AsUnsignedLongLong(item->bit_offset);
	if (bit_offset == (unsigned long long)-1 && PyErr_Occurred())
		return -1;
	unsigned long long bit_field_size =
		PyLong_AsUnsignedLongLong(item->bit_field_size);
	if (bit_field_size == (unsigned long long)-1 && PyErr_Occurred())
		return -1;

	struct drgn_lazy_type member_type;
	if (lazy_type_from_py(&member_type, (LazyType *)item,
			      builder->prog) == -1)
		return -1;
	struct drgn_error *err =
...
err_builder:
		...
		Program_hold_object(self, cached_members_obj);
	}
...
static int unpack_enumerator(struct drgn_enum_type_builder *builder,
			     PyObject *cached_enumerators_obj, bool is_signed)
{
	TypeEnumerator *item =
		(TypeEnumerator *)PyTuple_GET_ITEM(cached_enumerators_obj,
						   builder->enumerators.size);
	...
	const char *name = PyUnicode_AsUTF8(item->name);
	if (!name)
		return -1;

	struct drgn_error *err;
	if (is_signed) {
		long long svalue = PyLong_AsLongLong(item->value);
		if (svalue == -1 && PyErr_Occurred())
			return -1;
		err = drgn_enum_type_builder_add_signed(builder, name, svalue);
	} else {
		unsigned long long uvalue =
			PyLong_AsUnsignedLongLong(item->value);
...
DrgnType *Program_enum_type(Program *self, PyObject *args, PyObject *kwds)
{
	...
	if (enumerators_obj == Py_None) {
		...
		if (!cached_enumerators_obj)
			return NULL;
		size_t num_enumerators = PyTuple_GET_SIZE(cached_enumerators_obj);
		...
		for (size_t i = 0; i < num_enumerators; i++) {
			if (unpack_enumerator(&builder, cached_enumerators_obj,
					      is_signed) == -1)
...
err_builder:
		...
		Program_hold_object(self, cached_enumerators_obj);
	}
	...
	if (_PyDict_SetItemId(type_obj->attr_cache, &DrgnType_attr_tag.id,
	    ...
	    _PyDict_SetItemId(type_obj->attr_cache,
			      ...
			      cached_enumerators_obj ?
			      cached_enumerators_obj : Py_None) == -1)
		goto err_type;
...
static int unpack_parameter(struct drgn_function_type_builder *builder,
			    PyObject *cached_parameters_obj)
{
	TypeParameter *item = (TypeParameter *)PyTuple_GET_ITEM(cached_parameters_obj,
	...
	if (lazy_type_from_py(&parameter_type, (LazyType *)item,
			      builder->prog) == -1)
		return -1;
	struct drgn_error *err =
...
DrgnType *Program_function_type(Program *self, PyObject *args, PyObject *kwds)
{
	...
	PyObject *cached_parameters_obj = PySequence_Tuple(parameters_obj);
	if (!cached_parameters_obj)
		return NULL;
	size_t num_parameters = PyTuple_GET_SIZE(cached_parameters_obj);

	struct drgn_function_type_builder builder;
	drgn_function_type_builder_init(&builder, &self->prog);
	for (size_t i = 0; i < num_parameters; i++) {
		if (unpack_parameter(&builder, cached_parameters_obj) == -1)
			goto err_builder;
	}
...
err_builder:
	...
	Program_hold_object(self, cached_parameters_obj);
